...
Load `addresses.json.leveragedFarms` and `abis/FARM-ABI-SOURCES.json`; pick the chain and exact vault shape. **Live on Robinhood Chain (4663) since 2026-09-07** under `leveragedFarms.robinhoodMainnet`: the **UniV3DualVault** (vaultId 1) and **UniV3LeverageVault** (single-borrow, vaultId 2) share one LendingPool on the WETH/USDG fee100 pool. It is a **scaled soft launch** (200 USDG / 0.05 WETH reserve caps, single deployer key, no external audit yet) — size accordingly and still re-verify every address on-chain (VERIFY below) before approvals or transfers; the listed config must be confirmed against the live contracts. V4 vaults are not deployed. Sepolia is an explicitly selected test-only example, chainId **11155111**, using mocks; never fall back to it from 4663. The top-level address-book `verifiedAt` covers the pre-existing lending snapshot, not the farm records.
